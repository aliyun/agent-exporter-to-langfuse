...
LANGFUSE_ENV_FILE="$HOME/.qoder/langfuse.env"
...
DEFAULT_USER_ID="${QD_LANGFUSE_USER_ID:-}"
DEFAULT_TAGS="${QD_LANGFUSE_TAGS:-qoder}"
DEFAULT_MAX_CHARS="${QD_LANGFUSE_MAX_CHARS:-800000}"
DEFAULT_DEBUG="${QD_LANGFUSE_DEBUG:-true}"

prompt_input "User ID [${DEFAULT_USER_ID:-auto (Qoder user.name > OS username)}]: "
read -r INPUT_USER_ID
QD_LANGFUSE_USER_ID="${INPUT_USER_ID:-$DEFAULT_USER_ID}"

prompt_input "Tags, comma-separated [$DEFAULT_TAGS]: "
read -r INPUT_TAGS
QD_LANGFUSE_TAGS="${INPUT_TAGS:-$DEFAULT_TAGS}"

prompt_input "Max content chars [$DEFAULT_MAX_CHARS]: "
read -r INPUT_MAX_CHARS
QD_LANGFUSE_MAX_CHARS="${INPUT_MAX_CHARS:-$DEFAULT_MAX_CHARS}"

prompt_input "Debug logging (true/false) [$DEFAULT_DEBUG]: "
read -r INPUT_DEBUG
QD_LANGFUSE_DEBUG="${INPUT_DEBUG:-$DEFAULT_DEBUG}"
...
# 6a. Write dedicated env file
{
    ...
    [ -n "$QD_LANGFUSE_USER_ID" ]   && echo "export QD_LANGFUSE_USER_ID=\"$QD_LANGFUSE_USER_ID\""
    [ "$QD_LANGFUSE_TAGS" != "qoder" ]    && echo "export QD_LANGFUSE_TAGS=\"$QD_LANGFUSE_TAGS\""
    [ "$QD_LANGFUSE_MAX_CHARS" != "800000" ] && echo "export QD_LANGFUSE_MAX_CHARS=\"$QD_LANGFUSE_MAX_CHARS\""
    [ "$QD_LANGFUSE_DEBUG" = "false" ]    && echo "export QD_LANGFUSE_DEBUG=\"false\""
} > "$LANGFUSE_ENV_FILE"
info "Env vars written to $LANGFUSE_ENV_FILE."

# 6b. Add source line to shell profile (idempotent)
if ! grep -qF "langfuse.env" "$SHELL_RC" 2>/dev/null; then
...
<dict>
    ...
    <array>
        ...
        <string>-c</string>
        <string>
if [ -f "$HOME/.qoder/langfuse.env" ]; then
    . "$HOME/.qoder/langfuse.env"
    env | grep -E '^(LANGFUSE_|QD_LANGFUSE_)' | while IFS='=' read -r k v; do
