...
const LOG_DIR = join(homedir(), ".agent-exporter-to-langfuse", "logs");
const LOG_FILE = join(LOG_DIR, "cursor-hook.log");
...
function writeLog(level: string, args: unknown[]): void {
  ...
  try {
    ...
    try {
      process.stderr.write(`[cursor-langfuse] ${line}`);
      if (!logDirReady) {
        process.stderr.write(`[cursor-langfuse] log dir creation failed: ${LOG_DIR} — ${e}\n`);
      }
    } catch {}
  }
}
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    process.stdin.on("end", () => {
      ...
      try {
        ...
        reject(
          new Error(
            `failed to parse hook stdin: ${err instanceof Error ? err.message : String(err)}`,
          ),
        );
      }
    });
